...
| Метод | Путь | Описание |
|-------|------|----------|
| `GET` | `/api/v1/pools` | Список всех пулов (HTML-таблица в браузере, JSON для программного доступа; `?format=json` для принудительного JSON) |
...
├── scripts/
│   └── test_thrift.py             # Тестовый скрипт PyHive для Thrift HTTP transport
├── Dockerfile
├── Makefile
